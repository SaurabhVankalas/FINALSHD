...
def getPatientVisitData(regno):
        try:
            if request.method == 'POST':
                sql="select v.opdid,v.complaint,v.vdate,v.height,v.weight,v.bmi,v.systolic,v.diastolic,v.pulse,v.temp,v.rrate  from patient_registration p,opdvisit v where p.regno=v.regno AND v.regno='{}' order by v.vdate DESC".format(regno)
                cursor.execute(sql)
                result = cursor.fetchall()
                return result
        except Exception as e:
...
def getOpdConsultHistory(opdid):
    try:
        if request.method == 'POST':
            sql="select * from opdClinicalHistory  where opdid='{}'".format(opdid)
            cursor.execute(sql)
            result = cursor.fetchall()
            return result
    except Exception as e:
...
def getOpdConsultRefer(opdid):
    try:
        if request.method == 'POST':
            sql="select * from opdreferInfo  where opdid='{}'".format(opdid)
            cursor.execute(sql)
            result = cursor.fetchall()
            return result
    except Exception as e:
...
def getOpdConsultDiagnosis(opdid):
    try:
        if request.method == 'POST':
            sql="select * from opdDiagnosis  where opdid='{}'".format(opdid)
            cursor.execute(sql)
            result = cursor.fetchall()
            return result
    except Exception as e:
